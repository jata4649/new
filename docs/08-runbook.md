...
なお管理画面（オリパ詳細）でシードを公開する操作は、
公開前にこの再計算を自動で行い、一致しない場合は公開を中止する。
「検証せずに公開してしまう」ことが構造的に起こらないようにしてある。

---

## 11. シードの公開（リビール）

販売終了後、利用者が第三者として検証できるようにシードを公開する。

### 手順

1. 管理画面 → オリパ管理 → 該当オリパの詳細を開く
2. 「公正性の記録（コミット＆リビール）」の「シードを公開する」を押す
...
| 項目             | 内容                                                                   |
| ---------------- | ---------------------------------------------------------------------- |
| 実行できる時期   | 販売終了後のみ（完売・期間終了・アーカイブのいずれか）                 |
| 必要な権限       | `oripa:publish`                                                        |
| 取り消し         | **できない**。公開日時は一度入ったら変更できない                       |
| 公開前の自己点検 | 保存済みのスロット順から再計算し、コミットハッシュと照合する           |
| 記録             | `audit_logs` に `ORIPA_SEED_REVEAL` として残る（シード本体は残さない） |

### 公開後に利用者が見られるもの

オリパ詳細（`/oripas/[slug]`）の「検証の手順」に、
コミットハッシュ・シード・抽選順のランクコード列が並ぶ。
この 3 つから `sha256(campaignId + "|" + seed + "|" + tierCodes.join(","))` を計算し、
コミットハッシュと一致すれば、販売中に景品構成が差し替えられていないことが確かめられる。

**販売中にランクコード列を出してはならない。** 出すと次に何が出るかを計算できてしまう。
公開前は API もこの列を引かない（引いた値が別経路で漏れる余地を作らないため）。
...
| バッチ                  | 頻度 | 目的                                 | 遅れたときの影響                       |
| ----------------------- | ---- | ------------------------------------ | -------------------------------------- |
| `pnpm points:expire`    | 日次 | 期限切れポイントの記帳と口座の同期   | 無し（残高参照は常に期限で絞っている） |
| `pnpm cleanup:expired`  | 日次 | 期限切れセッション・冪等性キーの削除 | 無し（行数が増えるだけ）               |
| `pnpm points:reconcile` | 日次 | 台帳と口座の整合性検証               | 不整合の発見が遅れる                   |

いずれも `--dry-run` で対象件数だけを数えられる。
本番へ入れる前に必ず dry-run で件数を確認すること。

掃除バッチは 1 回あたり 10 万行で打ち切る（ロックの保持時間を抑えるため）。
打ち切った場合はその旨を出力するので、残りは次回の実行で処理される。

**掃除バッチは台帳・抽選・監査ログには触れない。**
これらは追記専用で、DB トリガが DELETE を拒否する。
もし掃除バッチがそれらを消そうとしたら、バッチ側が失敗する。
